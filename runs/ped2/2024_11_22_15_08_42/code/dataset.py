...
def get_dataset(seed=None):
    ...
    data_test = np.load('ped2_test_features.npy')
    ...
    return data_train, np.zeros(len(data_train)).astype(np.uint8), data_test, np.ones(len(data_test)).astype(np.uint8), id_to_type
